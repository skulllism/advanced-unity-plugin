        ...
        public void DrawConnection(Vector2 zoomScale)
        {
            ...
            if (childNodes != null)
            {
                for (int i = 0; i < childNodes.Count; i++)
                {
                    ...
                    //나보다 왼쪽에 있으면
                    if(rect.center.x >= childNodes[i].rect.center.x)
                    {
                        //나보다 밑에 있으면
                        if(rect.center.y >= childNodes[i].rect.center.y)
                        {
                            connections[i].Draw(zoomScale, new Vector2(rect.center.x + 5, rect.center.y - 5)
                                                         , new Vector2(childNodes[i].rect.center.x + 5, childNodes[i].rect.center.y - 5));
                        }
                        //나보다 위에 있으면
                        else
                        {
